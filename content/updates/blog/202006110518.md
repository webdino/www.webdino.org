...
While streaming, a web browser sequentially downloads video files encoded for a few seconds each from a CDN server, and play them smoothly in order. Web VideoMark measures the response time and throughput of each download, so we can analyze the trends and distributions to discover micro-perspective changes in the traffic quality that real user experience when they connect to a real services.
